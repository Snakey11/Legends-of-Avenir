...
static void CreatorBoonBaneDraw(CreatorProc* proc)
{
	...
	ApplyBGBox(gBG1MapBuffer,&gCreatorBoonBaneBoxTSA,10,0);
	
	// Draw the unit's constant base stats and growths.
	DrawUiNumber(&gBG0MapBuffer[3][13],TEXT_COLOR_GOLD,unit->maxHP);
	DrawUiNumber(&gBG0MapBuffer[5][13],TEXT_COLOR_GOLD,unit->pow);
	DrawUiNumber(&gBG0MapBuffer[7][13],TEXT_COLOR_GOLD,unit->unk3A); // Magic.
	DrawUiNumber(&gBG0MapBuffer[9][13],TEXT_COLOR_GOLD,unit->skl);
	DrawUiNumber(&gBG0MapBuffer[11][13],TEXT_COLOR_GOLD,unit->spd);
	DrawUiNumber(&gBG0MapBuffer[13][13],TEXT_COLOR_GOLD,unit->def);
	DrawUiNumber(&gBG0MapBuffer[15][13],TEXT_COLOR_GOLD,unit->res);
	DrawUiNumber(&gBG0MapBuffer[17][13],TEXT_COLOR_GOLD,unit->lck);
	
	DrawUiNumber(&gBG0MapBuffer[3][21],TEXT_COLOR_GOLD,charData->growthHP);
	DrawUiNumber(&gBG0MapBuffer[5][21],TEXT_COLOR_GOLD,charData->growthPow);
	DrawUiNumber(&gBG0MapBuffer[7][21],TEXT_COLOR_GOLD,MagCharTable[charData->number].growth); // Magic.
	DrawUiNumber(&gBG0MapBuffer[9][21],TEXT_COLOR_GOLD,charData->growthSkl);
	DrawUiNumber(&gBG0MapBuffer[11][21],TEXT_COLOR_GOLD,charData->growthSpd);
	DrawUiNumber(&gBG0MapBuffer[13][21],TEXT_COLOR_GOLD,charData->growthDef);
	DrawUiNumber(&gBG0MapBuffer[15][21],TEXT_COLOR_GOLD,charData->growthRes);
	DrawUiNumber(&gBG0MapBuffer[17][21],TEXT_COLOR_GOLD,charData->growthLck);
	...
	Text_Display(&baseHandle,&gBG0MapBuffer[1][13]);
	...
	Text_Display(&growthHandle,&gBG0MapBuffer[1][21]);
...
{
	BgMapFillRect(&gBG0MapBuffer[3][14],4,22-3,0);
	BgMapFillRect(&gBG0MapBuffer[3][22],4,22-3,0);
	...
	Text_Display(&mainBaseHandle,&gBG0MapBuffer[3+proc->commandIndex*2][15]);
	...
	Text_Display(&mainGrowthHandle,&gBG0MapBuffer[3+proc->commandIndex*2][23]);
	...
	if ( offset != -1 && offset != commandProc->commandDefinitionIndex )
	{
		...
		Text_Display(&otherBaseHandle,&gBG0MapBuffer[3+offset*2][15]);
		...
		Text_Display(&otherGrowthHandle,&gBG0MapBuffer[3+offset*2][23]);
	}
