...
@ Seems to know the chapter ID at 0x080BCFDC. Loses it somewhere around here?
@ Gets loaded again after that bl call...?
@ Is known again at 0x08034618. That's the function that gets the chapter data entry from the chapter ID.


@ldr r0, =#0x0202BCF0
@add r0, #0x0E
@ldrb r0, [ r0 ] @ r0 now has current chapter ID

@mov r1, #148 @ Btyes per entry of the chapter data table
@mul r0, r1

@ldr r1, =#0x0803462C
@ldr r1, [ r1 ] @ r1 now has current pointer to the chapter data table
@add r0, r1 @ r0 now has pointer to current chapter's data in the chapter data table

@mov r1, #112
@add r0, r1
@ldrh r0, [ r0 ] @ r0 now has text ID
